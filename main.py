...
class Client:
        ...
        while self.server.online:
            try:
                ...
                if not len(data):
                    ...
                    raise ClientDisconnectErr(f"Invalid packet format from user at {self.ip}")
            ...
            for obj in os.scandir("/home/servers/software/libs/"):
                try:
                    if obj.is_file():
                        ...
                        print(f"{fileparts}")
                        if fileparts[len(fileparts)-1]=="bin":
    ...
    def get_file(self, item):
        if not len(item)==PacketSizes["FILE_WRITE_START"]:
            ...
            return
        print(f"{item}")
        self.file_name=str(bytes(item[:9]), 'utf-8').split('\0', maxsplit=1)[0]
        ...
        try:
            ...
            if os.path.isfile(f"/home/servers/software/usr/{file_wext}.bin"):
                ...
                self.send([ControlCodes["SERVER_ERROR"], ErrorTypes["SERVER_FNF_ERROR"]])
                return
            ...
            with open(filepath, "rb") as f:
                ...
                self.server.emit_log(logging.INFO, f"comparing SHA-1 digests...\nDevice: {hex(int.from_bytes(file_sha1,'big'))}\nHosted {hex(int.from_bytes(self.sha1_hosted, 'big'))}")
    ...
    def file_send_continue(self):
        if self.bytes_remain==0:
            odata=[]
            odata += self.parse_string(PaddedString(self.file_name, 8, chr(0)))
            odata += [self.file_type]
            odata += list(self.sha1_hosted)
